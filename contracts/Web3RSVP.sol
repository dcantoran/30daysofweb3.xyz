...
contract Web3RSVP {
    ...
    // exposes data about the new event like the owner, max cap., event owner, deposit amt, etc.
    ...
    // exposes data about the user who RSVP's and the event they RSVP's to
    event NewRSVP(
        bytes32 eventId,
    ...
    function createNewRSVP(bytes32 eventId) external payable {
        ...
        // require that msg.sender isn't already in myEvent.confirmedRSVPs AKA hasn't already RSVPd
    ...
    function confirmedAttendee(bytes32 eventId, address attendee) public {
        ...
        // require that msg.sender is the owner of the event - only the host should be to check people in
        require(msg.sender == myEvent.eventOwner, "NOT AUTHORIZED");

        // require that attendee trying to check in actually RSVPd
        address rsvpConfirm;
    ...
    function withdrawUnclaimedDeposits(bytes32 eventId) external{
        ...
        // check that the paidOut boolean still equals false AKA that the money hasnn't already been paid out yet
        require(!myEvent.paidOut, "ALREADY PAID");

        // make sure it's been 7 days past myEvent.eventTimestamp
        require(
